...
router.get("/postagens/add", (req, res) => {
  Categoria.find()
    ...
    .catch((err) => {
      req.flash("error_msg", "Houve um erro ao o formulário");
      res.redirect("/admin");
    });
});
...
router.post("/postagens/nova", (req, res) => {
  ...
  if (erros.length > 0) {
    ...
    new Postagem(novaPostagem)
      ...
      .catch((err) => {
        req.flash("error_msg", "Houve erro durante o salvamento da postagem");
        res.redirect("/admin/postagens");
      });
  }
});
...
router.get("/postagens/edit/:id", (req, res) => {
  ...
  Postagem.findOne({ _id: req.params.id })
    .lean()
    .then((postagem) => {
      //Pesquisa no mongo pela categoria
      Categoria.find()
        .lean()
        .then((categorias) => {
        ...
        .catch((err) => {
          req.flash(
            "error_msg",
            "Houve um erro ao listar as categorias de edição"
          );
          res.redirect("/admin/postagens");
        });
    })
